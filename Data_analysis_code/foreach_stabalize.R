...
parallel4 <-read.csv(file="parallel4.csv", sep="\t", header=F)
parallel4
############################################################################################
############################################################################################
...
#correlationSim <- list()
  ...
  foreach(m=1:length(list)) %:% # we loops through all unique pairs (order does not matter since cor.sim(X,Y)=cor.sim(Y,X))
