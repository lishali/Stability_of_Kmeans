...
k.stabalize <- function(Df, k, num.samples, ratio.subsample){ 
  ...
  for (j in 1:num.samples){ #I will calculate kmeans for each of the num.samples number of subsamples.   
    ...
    k.means <- kmeans(subsample[7:ncol(Df)],k) #this was hardcoded in since the lingBinarize dataset's questions start on column 7 to the last ncol(Df), and this is the same for its subsamples
